## Includes cohort 1 data only, no second grade
...
Production <- rbind(L2_45_Prod, HS_45_Prod, L2_78_Prod, HS_78_Prod) %>% 
  group_by(Part_ID, Age_Group, Speaker_Group, Verb_Type) %>%
...
Master$Verb_Type <- factor(Master$Verb_Type, levels = c("Regular", "Irregular"))


# Aggregate model
Regularity_Boxplot <- Master %>% 
  ggplot(aes(x = Age_Group, y = Ratio, fill = Task)) +
  geom_boxplot() +
  facet_grid(vars(Verb_Type), vars(Speaker_Group)) +
  scale_y_continuous(breaks = seq (0, 100, 20),
                     limits = c(0, 100)) +
  scale_fill_manual(values = c("#56B4E9", "#F0E442")) +
  labs(x = "Age group", y = "Percentage of subjunctive responses", title = "Distribution of Subjunctive Use by Group, Task, and Regularity") +
...
ggsave(filename = here("Manuscripts", "Subjunctive Manuscript (SHLL)", "Graphs", "SHLL Figure 4.pdf"),
       plot = Regularity_Boxplot,
       device = "pdf",
       width = 6.5,
       height = 3.5,
       units = "in")
